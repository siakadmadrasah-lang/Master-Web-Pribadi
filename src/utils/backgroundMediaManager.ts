...
class BackgroundMediaManager {
  ...
  // Ultra-lightweight static 44-byte silent WAV audio anchor for Android Chrome OS Audio Focus
  ...
  private initNativeAudioElement() {
    ...
    try {
      if (!this.silentAudioElement) {
        const audio = new Audio(BackgroundMediaManager.SILENT_WAV_DATA_URI);
        audio.loop = true;
        audio.volume = 0.05;
